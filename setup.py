...
__version__ = '?'
exec(open(os.path.join("lib", "fcgiev", "release.py")).read())
system_config_h = os.path.join("src", "system_config.h")
libraries = [] # example: [ ['tokyocabinet', ['tchdb.h']] ]
...
class build_ext(_build_ext):
  ...
  def finalize_options(self):
    ...
    if not isinstance(self.define, list):
      self.define = []
    self.define.append(('FCGIEV_VERSION', '"%s"' % __version__))
  ...
  def _write_system_config_h(self):
    ...
    try:
      try:
        ...
        f.write("#ifndef FCGIEV_SYSTEM_CONFIG_H\n")
        f.write("#define FCGIEV_SYSTEM_CONFIG_H\n\n")
...
setup(name = 'fcgiev',
      ...
      author = '${PROJECT_AUTOR_NAME}',
      author_email = '${PROJECT_AUTOR_EMAIL}',
      url = '',
      distclass = Dist,
      ext_modules = [
        Extension('_fcgiev',
